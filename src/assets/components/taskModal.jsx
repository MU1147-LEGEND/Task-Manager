...
const TaskModal = ({ changeModalState, handeSubmit, task, setTask, editTask }) => {
    
    // Sync editTask with task when editTask is present
    useEffect(() => {
        if (editTask) {
            setTask(editTask);
        }
    }, [editTask, setTask]); // Depend on editTask and setTask
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); handeSubmit(); }} className="flex flex-col py-10 lg:py-3 gap-10 lg:gap-7">
                            ...
                            <div className="priority flex flex-col">
                                ...
                                <select
                                    ...
                                    id="priority"
                                    value={task.priority || ""}
                                    className="focus:outline-none bg-inherit lg:w-2/3 border-none appearance-none pt-2 px-4" >
                            ...
                            <div className="buttons flex items-center justify-around absolute bottom-4 left-0 right-0">
                                <button
                                    ...
                                    className="bg-red-600 text-white py-2 px-5 rounded-lg hover:bg-red-700 transition-all duration-300">
                                    Close
                                </button>

                                <button
                                    type="submit"
                                    className="bg-blue-600 text-white py-2 px-5 rounded-lg hover:bg-blue-800 transition-all duration-300">
                                    Save
                                </button>
                            </div>
                        </form>
